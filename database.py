...
def init_db():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if not _col_exists('inventory', 'unit_type'):
            cursor.execute("ALTER TABLE inventory ADD COLUMN unit_type TEXT NOT NULL DEFAULT 'piece'")
        ...
        if cursor.execute("SELECT COUNT(*) FROM inventory").fetchone()[0] == 0:
            cursor.executemany(
                "INSERT INTO inventory (name, name_ta, price, image_url, category, stock) VALUES (?, ?, ?, ?, ?, ?)",
                [
                    ("Fresh Milk",        "பசுவின் பால்",     2.99, "/static/images/milk.png",    "General", 50),
                    ("Whole Wheat Bread", "கோதுமை ரொட்டி",    3.49, "/static/images/bread.png",   "General", 30),
                    ("Red Apples",        "ஆப்பிள்",          1.99, "/static/images/apples.png",  "General", 100),
                    ("Bananas",           "வாழைப்பழம்",       0.99, "/static/images/bananas.png", "General", 80),
                ]
            )
