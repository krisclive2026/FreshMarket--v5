...
class Item(BaseModel):
    name: str
    price: float
    quantity: float                    # integer count for piece items, kg for weight items
    unit: Optional[str] = None         # "kg" for weight-based lines, None/"" for piece-based
...
class InventoryItem(BaseModel):
    ...
    def resolved_name(self) -> str:
        """Name is optional in either language; at least one must be set."""
        name    = (self.name or "").strip()
        name_ta = (self.name_ta or "").strip()
        if not name and not name_ta:
            raise HTTPException(status_code=400, detail="Enter the item name in English or Tamil.")
        return name or name_ta

    def resolved_unit_type(self) -> str:
        ut = (self.unit_type or "piece").strip().lower()
        return ut if ut in ("piece", "weight") else "piece"

class CategoryCreate(BaseModel):
    name: str

class BarcodePrintRequest(BaseModel):
    items: List[dict]   # each: {barcode, name, price}
...
def _fmt_qty(quantity: float, unit: Optional[str] = None) -> str:
    """Format a cart line's quantity for receipts. Piece-based lines print as
    plain whole numbers; weight-based lines print trimmed decimals with a kg suffix."""
    q = float(quantity)
    if unit == "kg":
...
def print_thermal_receipt(sale_id: int, cart) -> bool:
    ...
    try:
        ...
        try:
            ...
            _wline(hPrinter, f"{'ITEM':<17} {'QTY':>6} {'PRICE':>8} {'AMT':>8}")
            _write(hPrinter, ESC_BOLD_OFF)
            _wline(hPrinter, "-" * W)
            ...
            for item in cart.items:
                ...
                name = item.name[:17]
                qty_text = _fmt_qty(item.quantity, item.unit)
                line = f"{name:<17} {qty_text:>6} {item.price:>8.2f} {line_total:>8.2f}"
...
def print_barcode_label(barcode: str, name: str, price: float) -> bool:
    """Print a single barcode label on the 80mm thermal receipt printer."""
    W = RECEIPT_WIDTH

    def _build_escpos_barcode(barcode_val: str, name: str, price: float) -> bytes:
        buf = bytearray()
        buf += ESC_INIT
        ...
        # Price line
        price_line = f"Rs. {price:.2f}"
        buf += (price_line + "\n").encode("cp437", errors="replace")

        # Small gap before barcode
        buf += b'\x1bJ\x08'

        # GS k — print barcode (new format: GS k <type> <len> <data>)
        # type 0x49 = CODE128; data must be prefixed with {B to select subset B
        barcode_bytes = ('{B' + barcode_val).encode("ascii", errors="replace")
        barcode_len = len(barcode_bytes)

        # Set barcode height: GS h n
        buf += b'\x1dh\x50'
        # Set barcode width: GS w n  (2 = standard)
        buf += b'\x1dw\x02'
        # HRI position: GS H 2  (below barcode)
        buf += b'\x1dH\x02'
        # HRI font: GS f 0
        buf += b'\x1df\x00'

        # GS k CODE128: 0x1d 0x6b 0x49 <len> <data>
        buf += b'\x1dk\x49' + bytes([barcode_len]) + barcode_bytes

        # Feed and cut
        buf += b'\n'
        buf += ESC_FEED_LINES(4)
        buf += ESC_CUT

        return bytes(buf)

    raw = _build_escpos_barcode(barcode, name, price)
    ...
    try:
        ...
        hPrinter = win32print.OpenPrinter(printer_name)
        try:
            win32print.StartDocPrinter(hPrinter, 1, ("Barcode Label", None, "RAW"))
            win32print.StartPagePrinter(hPrinter)
            ...
            win32print.ClosePrinter(hPrinter)
        print(f"[PRINTER] Barcode label sent: {barcode} / {name}")
        return True
    except Exception as e:
        print(f"[PRINTER] win32print barcode failed: {e} — trying serial.")
    ...
    try:
        ...
        port = _find_serial_printer()
        if not port:
            print("[PRINTER] No serial printer for barcode.")
            return False
        with serial.Serial(port, baudrate=9600, timeout=2) as ser:
            ser.write(raw)
        print(f"[PRINTER] Barcode label sent via serial: {port}")
        return True
    except Exception as e:
        print(f"[PRINTER] Serial barcode failed: {e}")
        return False
...
def _build_receipt_text(sale_id: int, cart) -> str:
    ...
    for item in cart.items:
        ...
        name = item.name[:17]
        qty_text = _fmt_qty(item.quantity, item.unit)
        lines.append(f"{name:<17} {qty_text:>6} {item.price:>8.2f} {line_total:>8.2f}")
    lines.append("-" * W)
    if show_subtotal:
...
def checkout(cart: Cart):
    ...
    return {
        ...
        "items": [{"name": i.name, "price": i.price, "quantity": i.quantity, "unit": i.unit, "discount": i.discount} for i in cart.items],
...
@app.post("/api/print-barcodes")
def print_barcodes(req: BarcodePrintRequest):
    if not req.items:
    ...
    for item in req.items:
        ...
        if not barcode:
            results.append({"name": name, "ok": False, "error": "No barcode"})
            continue
        ok = print_barcode_label(barcode, name, price)
...
def add_inventory(item: InventoryItem):
    ...
    unit_type = item.resolved_unit_type()
    with get_db() as db:
        cursor = db.cursor()
        cursor.execute(
